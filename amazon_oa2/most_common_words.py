...
Amazon is partnering with the linguistics department at a local university to analyze important works of English 
  literature and identify patterns in word usage across different eras. 
To ensure a cleaner output. the linguistics department has provided a list of commonly used words (e.g., “an”, “the”. etc.) 
...
def solution(text, words_to_exclude):
  words_to_exclude_set = set(words_to_exclude)
  for c in "!?,;:.'":
    text = text.replace(c, " ")
  
  count = collections.Counter(word for word in text.split())

  ans, max_count = [], 0
  for word in count:
    if count[word] > max_count and word not in words_to_exclude_set:
      if ans:
        ans = []
...
print(solution("Jack and Jill went to the market to buy bread and cheese. Cheese is Jack's and Jill's favorite food.",
["and", "he", "the", "to", "is", "Jack", "Jill"]))
print(solution("a, a, a, a, b,b,b,c, c", ["a"]))
